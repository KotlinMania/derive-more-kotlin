// port-lint: tests tests/boats_display_derive.rs
...
/**
 * Tests mirroring upstream `tests/boats_display_derive.rs`.
 *
 * Upstream uses `#[derive(Display)]` with custom `#[display("...")]` format
...
class BoatsDisplayDeriveTest {
    ...
    data class RecordError(val code: UInt) {
    ...
    data class TupleError(val code: Int) {
        override fun toString(): String = "Error code: $code"
    }
    ...
    sealed class EnumError {
        data class StructVariant(val code: Int) : EnumError() {
            override fun toString(): String = "Error code: $code"
        }

        data class TupleVariant(val value: String) : EnumError() {
