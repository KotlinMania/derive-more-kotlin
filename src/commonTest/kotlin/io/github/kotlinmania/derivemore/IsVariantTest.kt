// port-lint: tests tests/is_variant.rs
...
/**
 * Tests mirroring upstream `tests/is_variant.rs`.
 *
 * Upstream uses `#[derive(IsVariant)]` to generate `is_<variant>()` methods.
...
class IsVariantTest {
    ...
    sealed class Either<out TLeft, out TRight> {
        data class Left<TLeft>(val value: TLeft) : Either<TLeft, Nothing>()

        data class Right<TRight>(val value: TRight) : Either<Nothing, TRight>()
    }
    ...
    sealed class WithConstraints<out T> {
        data class One<T>(val value: T) : WithConstraints<T>()
    ...
    sealed class KitchenSink<out T1, out T2> {
        data class Left<T1>(val value: T1) : KitchenSink<T1, Nothing>()
        data class Right<T2>(val value: T2) : KitchenSink<Nothing, T2>()
        data class OwnBoth<T1, T2>(val left: T1, val right: T2) : KitchenSink<T1, T2>()
