# Claude Code Project Instructions — derive-more-kotlin

## Project Overview

This is **derive-more-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`derive_more`](https://crates.io/crates/derive_more).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/derive_more/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/derive_more>
